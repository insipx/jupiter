  ...
  resources = {
    ...
    deployments.${app} = {
      ...
      spec = {
        ...
        template = {
          metadata.labels.app = app;
          spec = {
            # Image runs as non-root UID 1000 since v3.4.0. fsGroup makes the
            # kubelet chown the (root-owned) Longhorn volume to group 1000 so
            # SQLite can create /data/wealthfolio.db.
            securityContext = {
              runAsUser = 1000;
              runAsGroup = 1000;
              fsGroup = 1000;
    ...
    ingressroute.wealthfolio = {
      ...
      spec = {
        ...
        routes = [
          {
            match = "Host(`wealth.${flake.lib.hostname}`)";
            kind = "Rule";
            services = [
              {
                name = app;
    ...
    ingressroute.wealthfolio-external = {
      ...
      spec = {
        ...
        routes = [
          {
            match = "Host(`wealth.${flake.lib.hostname}`) || Host(`wealth.${flake.lib.external-hostname}`)";
            kind = "Rule";
            services = [
              {
                name = app;
